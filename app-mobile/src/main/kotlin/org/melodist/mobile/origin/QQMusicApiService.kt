...
open class QQMusicApiService : Service() {
    ...
    private val binder = object : IQQMusicApi.Stub() {
            ...
            return try {
                when (act) {
                    ...
                    "playSongIdAtIndex" -> {
                        ...
                        runOnMain {
                            if (index >= 0) {
                                ...
                                val targetSong = if (!songIdList.isNullOrEmpty() && index < songIdList.size) {
                                    val targetSongId = songIdList[index]
                                    currentPlaylist.find { it.songMid == targetSongId || it.songId.toString() == targetSongId }
                                        ?: org.melodist.data.UserLibraryCacheManager.favoriteSongsFlow.value.find { it.songMid == targetSongId || it.songId.toString() == targetSongId }
                                        ?: org.melodist.data.LocalMusicManager.getScannedSongs().find { it.songMid == targetSongId || it.songId.toString() == targetSongId }
                                        ?: org.melodist.data.download.DownloadManager.completedTasks.value.map { it.song }.find { it.songMid == targetSongId || it.songId.toString() == targetSongId }
                                } else null
                    ...
                    "seekPos", "seek" -> {
                        ...
                        val finalPos = if (pos >= 0L) pos else {
                    ...
                    "getSongList", "getUserSongList" -> {
                        ...
                        val songs: List<Song> = when {
                            folderType == 100 || folderId == "100" -> {
                                val downloaded = org.melodist.data.download.DownloadManager.completedTasks.value.map { it.song }
                                val scanned = org.melodist.data.LocalMusicManager.getScannedSongs()
                    ...
                    "getPlayMode", "getLoopMode" -> {
                        val mode = when (PlaybackManager.loopMode.value) {
                            org.melodist.playback.PlaybackLoopMode.ListRepeat -> 0
                            org.melodist.playback.PlaybackLoopMode.SingleRepeat -> 1
                            org.melodist.playback.PlaybackLoopMode.Shuffle -> 2
                        }
        ...
        override fun executeAsync(action: String?, params: Bundle?, callback: IQQMusicApiCallback?) {
        ...
        override fun registerEventListener(events: MutableList<String>?, listener: IQQMusicApiEventListener?): Bundle {
            val callingUid = android.os.Binder.getCallingUid()
            val callingPid = android.os.Binder.getCallingPid()
            val callingPkg = packageManager.getNameForUid(callingUid)
            ...
            if (listener != null) {
                ...
                serviceScope.launch {
                    try {
                        ...
                        listener.onEvent(EVENT_PLAY_STATE_CHANGED, Bundle().apply {
                            putInt("code", 0)
                            putInt("data", state)
                            putInt("playState", state)
                        })
                        val songJson = buildSongJson(PlaybackManager.currentSong.value)
                        listener.onEvent(EVENT_PLAY_SONG_CHANGED, Bundle().apply {
                            putInt("code", 0)
                            putString("data", songJson)
                            putString("playSong", songJson)
                            putString("song", songJson)
        ...
        override fun unregisterEventListener(events: MutableList<String>?, listener: IQQMusicApiEventListener?): Bundle {
            if (listener != null) {
    ...
    private fun dispatchSystemEvent(event: String, bundle: Bundle) {
    ...
    private fun buildSongJson(song: Song?): String {
        ...
        val singerObj = JSONObject().apply {
