...
object PlaybackMediaLoader {
    ...
    suspend fun resolveTarget(
    ...
    ): PlaybackTargetResult = withContext(Dispatchers.IO) {
        // 1. 检查纯本地文件
        ...
        if (song.songMid.startsWith("webdav_")) {
            ...
            val localFile = if (server != null && relativeHref.isNotBlank()) {
        ...
        val playUrlInfo = if (higherStereoTier != null) {
            ...
            val cachedInfo = if (prefetchedUrlInfo?.first == song.songMid) {
        ...
        if (playUrlInfo != null && !playUrlInfo.url.isNullOrBlank()) {
            ...
            val dsFactory = if (shouldCache || isCached || isPrecached || !MelodistCacheManager.currentProfile.isTvDevice) {
