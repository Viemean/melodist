...
class AudioHeaderSnifferTest {
    @Test
    fun `test AudioHeaderSniffer enrichQualityOptions with real track`() =
        runBlocking {
            ...
            for (q in queries) {
                ...
                for (s in songs.take(2)) {
                    ...
                    if (availableFlac.isNotEmpty()) {
                        ...
                        println(
                            "嗅探后 SQ: size=${sqOptionAfter.sizeBytes}, bitDepth=${sqOptionAfter.bitDepth}, sampleRate=${sqOptionAfter.sampleRateHz}, bitrate=${sqOptionAfter.bitrate}",
                        )
