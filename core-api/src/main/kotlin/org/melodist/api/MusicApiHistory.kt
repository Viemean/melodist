 ...
 */
enum class RecentHistoryType(val typeCode: Int) {
    Song(2),
    Album(3),
    Playlist(4),
    Radio(5),
...
fun buildDeleteRecentHistoryPayload(
    ...
    uin: String = "",
    authst: String = "",
): String = buildDeleteRecentHistoryBatchPayload(
...
fun parseRecentSongsResponse(jsonStr: String): RecentHistoryResult<RecentSongItem> {
    ...
    val updateTime = innerData["updateTime"]?.jsonPrimitive?.longOrNull
        ?: outerData["updateTime"]?.jsonPrimitive?.longOrNull
        ?: 0L
    val songList = innerData["songList"]?.jsonArray
        ?: outerData["songList"]?.jsonArray
        ?: return RecentHistoryResult(emptyList(), updateTime)

    val items = songList.mapNotNull { itemElem ->
...
fun parseRecentAlbumsResponse(jsonStr: String): RecentHistoryResult<RecentAlbumItem> {
    ...
    val updateTime = innerData["updateTime"]?.jsonPrimitive?.longOrNull
        ?: outerData["updateTime"]?.jsonPrimitive?.longOrNull
        ?: 0L
    val albumList = innerData["albumList"]?.jsonArray
    ...
    val items = albumList.mapNotNull { itemElem ->
        ...
        if (albumMid.isBlank() && albumId == 0L) null
        else RecentAlbumItem(
            albumId = albumId,
            albumMid = albumMid,
            albumName = albumName,
...
fun parseRecentPlaylistsResponse(jsonStr: String): RecentHistoryResult<RecentPlaylistItem> {
    ...
    val updateTime = innerData["updateTime"]?.jsonPrimitive?.longOrNull
        ?: outerData["updateTime"]?.jsonPrimitive?.longOrNull
        ?: 0L
    val folderList = innerData["geDanList"]?.jsonArray
...
suspend fun MusicApiService.reportRecentHistory(
    ...
    val uin = getEffectiveUin()
    val authst = getEffectiveAuthst()
    val payload = buildReportRecentHistoryPayload(
 ...
 * 批量删除最近播放记录
 */
suspend fun MusicApiService.deleteRecentHistoryBatch(
    items: List<RecentDeleteItem>,
): Boolean = withContext(Dispatchers.IO) {
